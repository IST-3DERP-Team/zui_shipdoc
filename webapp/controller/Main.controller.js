    ...
    function (Controller,JSONModel,MessageBox,Common,Filter,FilterOperator) {
        ...
        return Controller.extend("zuishipdoc.controller.Main", {
            ...
            onInit: function () {
                ...
                this._oModel.read("/ShipModeSHSet", {
                    success: function (oData, oResponse) {
                        ...
                        me.getOwnerComponent().getModel("SHIPMODE_MODEL").setProperty("/rows", oData.results);
                    },
                    error: function (err) { }
                });
                ...
                this._oModel.read("/StatusSHSet", {
                    success: function (oData, oResponse) {
                        ...
                        me.getOwnerComponent().getModel("STATUS_MODEL").setProperty("/rows", oData.results);
                    },
                    error: function (err) { }
                });    
            ...
            onEdit: function (oEvent) {
                this.getOwnerComponent().getModel("UI_MODEL").setProperty("/action", "EDIT");
                this.navToDetail();
            },
            ...
            navToDetail: function(oEvent) {
                if (this.getOwnerComponent().getModel("UI_MODEL").getData().action !== "NEW") {
                    this.getOwnerComponent().getModel("DELVDATA_MODEL").setData({
                        header: this.byId("mainHeaderTab").getModel().getData().rows.filter(item => item.DLVNO === this.getOwnerComponent().getModel("UI_MODEL").getData().activeDlv)[0],
                        detail: this.byId("mainDetailTab").getModel().getData().rows.filter(item => item.DLVNO === this.getOwnerComponent().getModel("UI_MODEL").getData().activeDlv),
                    })
                }
